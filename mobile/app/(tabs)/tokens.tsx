import { View, Text, StyleSheet, ScrollView, Pressable, TextInput, Alert, ActivityIndicator, Image, Dimensions } from 'react-native';
...
export default function TokensScreen() {
  ...
  // Initialize default amount when charity is selected
  ...
  const handleDonate = async () => {
    ...
    if (amount < selectedCharity.cost_tokens) {
      Alert.alert(
        t.tokens.donationError,
        `${t.tokens.minDonation} ${selectedCharity.cost_tokens} tokens`
      );
      return;
    }
    ...
    if (amount > userTokens) {
      Alert.alert(
        t.tokens.donationError,
        `${t.tokens.yourBalance}: ${userTokens} tokens`
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(t.tokens.donationError, err.message);
    } finally {
      setDonating(false);
    }
  };
  ...
  if (selectedCharity) {
    ...
    const isValidAmount = amount >= selectedCharity.cost_tokens && amount <= userTokens;
            ...
            <Animated.View entering={FadeInDown.delay(400)} style={styles.amountSection}>
              ...
              <View style={styles.donationInputContainer}>
                <TextInput
                  ...
                  onChangeText={(text) => {
                    ...
                    setSelectedMultiplier(null); // Odznacz multiplier gdy user pisze
                  }}
                />
                      ...
                      <Pressable
                        ...
                        onPress={() => {
                          if (!isDisabled) {
                            ...
                            setDonationAmount(calculatedAmount.toString());
                          }
                        }}
                        disabled={isDisabled}
                      >
              ...
              <View style={styles.minTokensInfo}>
                ...
                <Text style={styles.minTokensInfoText}>
                  Minimum {selectedCharity.cost_tokens} tokens required for this cause
                </Text>
              </View>
            </Animated.View>
                ...
                >
                  {donating ? (
                    ...
                    <>
                      ...
                      <Text style={styles.donateButtonTextLarge}>Donate tokens</Text>
                    </>
                  )}
                </LinearGradient>
              </Pressable>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(100)}>
          ...
          >
            <Text style={styles.balanceBannerLabel}>{t.tokens.yourBalance}</Text>
            <View style={styles.balanceBannerRow}>
              <Coins size={40} color="#ffffff" />
              <Text style={styles.balanceBannerAmount}>{userTokens}</Text>
            </View>
          </LinearGradient>
        </Animated.View>

        <ScrollView style={styles.charitiesList} showsVerticalScrollIndicator={false}>
          <Text style={styles.listTitle}>Support a cause</Text>
